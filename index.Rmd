...
*Cactaceae* are only native to the Western Hemisphere, excluding one species in Africa [@Sanchez2010]. However, members of the family can be found globally thanks to nonnative introductions and agriculture. Of the current studies, most data points are reported in studies occuring in the Western Hemisphere, where they are native (Figure 3). Most reported values and studies occur in field experiments, and field experiments are more likely to report more values per study (Figure 4).
...
mp1 <- mp1 + 
  geom_point(aes(x=cactlat, y=cactlon, color=country), size=4, alpha=1/10) +
  labs(colours = "#514E4E", title = "A", x="Latitude", y="Longitude") + 
...
mp2 <- mp2 + 
  geom_point(aes(x=allcactlon, y=allcactlat), color="#ACDF8F", size=1, alpha=1/10) +
...
Figure 3: Studies on cactus reproduction are more prevalent and report more values in the Western Hemisphere (Figure 3A). Cacti are found globally, but all occurences in the Eastern Hemisphere are either agricultural or introduced populations (Figure 3B).
...
Overall health is another important factor to consider when exploring potential study species, as health can determine an individual's likelihood to flower and fruit. *C. acanthocarpa* and *C. echinocarpa* are left skewed, with most individuals having health ratings of "4" or "5", whereas *O. basilaris* has an even distribution of health classes amoung all individuals (Figure 7). 

```{r Health boxplot, warning=FALSE, echo=FALSE}
ggplot(baseline, aes(x = health_index)) +
  geom_histogram(bins = 15) + labs(x = "Number of individuals in each health class by species", y = "Health class") +
  facet_grid(~species) +
  theme_minimal()
```
